...
module.exports = function(file) {
    ...
    lines = lines.map(function(line, index, array) {
        ...
        //// terminate pipe by inserting .value() after last |>
        //line = transform.append_pipe_value(line, index, array);
...
// ------------------  refactor below into transform.js ---------------
        ...
        /**
         * Replaces an `elsif` with `} else if () {`
         * @param  {string} $0  Entire match
         * @param  {string} $1  Matches `condition`
         */
        line = line.replace(/elsif (?!\s*\()(.+)/, function($0, $1) {
            // if `else if` is inside a string, bail out
            if (insideString("elsif", line)) return $0;
            return '} else if (' + $1 + ') {';
        });
